...
from bert.sentiment_modeling import BertForSpanAspectExtraction, DistilBertForSpanAspectExtraction, SpanBertForSpanAspectExtraction
...
def run_train_epoch(args, global_step, model, param_optimizer, train_dataloader,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        if args.distilled:
            # distill, does not use segment_ids
            loss = model(input_ids=input_ids,  attention_mask=input_mask, start_positions=start_positions, end_positions=end_positions)
        else:
            # bert
...
def main():
    ...
    parser.add_argument("--distilled", default=False, action='store_true', help="Use DistilledBERT model")
    ...
    if args.distilled:
        model = DistilBertForSpanAspectExtraction(bert_config)
    else:
        # model = SpanBertForSpanAspectExtraction(bert_config)
