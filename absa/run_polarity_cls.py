...
from absa.utils import read_absa_data, convert_absa_data, convert_absa_data_polarity, convert_examples_to_features, convert_examples_to_features_polarities, \
    RawFinalResult, RawFinalResultPolarity, wrapped_get_final_text, id_to_label
...
def read_train_data(args, tokenizer, logger):
    ...
    train_examples = convert_absa_data_polarity(dataset=train_set, verbose_logging=args.verbose_logging)
    
    train_features = convert_examples_to_features_polarities(train_examples, tokenizer, args.max_seq_length, args.verbose_logging, logger)
    num_train_steps = int(
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_labels, all_label_masks)
...
def read_eval_data(args, tokenizer, logger):
    ...
    eval_examples = convert_absa_data_polarity(dataset=eval_set, verbose_logging=args.verbose_logging)
    eval_features = convert_examples_to_features_polarities(eval_examples, tokenizer, args.max_seq_length,
    ...
    eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_masks, all_example_index)
    if args.local_rank == -1:
...
def run_train_epoch(args, global_step, model, param_optimizer, train_dataloader,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        input_ids, input_mask, segment_ids, labels, label_masks = batch
        loss = model('train', input_mask, input_ids=input_ids, token_type_ids=segment_ids, labels=labels, label_masks=label_masks)
...
def metric_polarity(polarity, gold_polarity):
...
def eval_absa(all_examples, all_features, all_results, do_lower_case, verbose_logging, logger):
    ...
    for (feature_index, feature) in enumerate(all_features):
        ...
        if result.span_masks:
            pred_terms.append(example.term_texts)
            pred_polarities.append(id_to_label[result.cls_pred])
        ...
        for term, polarity in zip(pred_terms, pred_polarities):
            common += metric_polarity(polarity, example.polarity)
        retrieved += 1
        relevant += 1
...
def evaluate(args, model, device, eval_examples, eval_features, eval_dataloader, logger, write_pred=False):
    ...
    for batch in eval_dataloader:
        ...
        input_ids, input_mask, segment_ids, label_masks, example_indices = batch
        with torch.no_grad():
        ...
        for j, example_index in enumerate(example_indices):
            cls_pred = cls_logits[j].detach().cpu().numpy().argmax().tolist()
            span_masks = label_masks[j].detach().cpu().tolist()
            eval_feature = eval_features[example_index.item()]
            unique_id = int(eval_feature.unique_id)
            all_results.append(RawFinalResultPolarity(unique_id=unique_id, cls_pred=cls_pred, span_masks=span_masks))
