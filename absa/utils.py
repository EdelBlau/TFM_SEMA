...
class PolarityExample(object):
    ...
    def __repr__(self):
        ...
        if self.term_texts:
            s += ", term_texts: {}".format(self.term_texts)
        if self.polarities:
            s += ", polarity: {}".format(self.polarity)
        return s

class MultilabelClsDataset(Dataset):

    def __init__(self, dataframe, tokenizer, max_len):
        self.tokenizer = tokenizer
        self.data = dataframe
        self.comment_text = dataframe.match
...
class InputFeaturesPolarity(object):
    """A single set of features of data."""
...
def convert_examples_to_features_polarities(examples, tokenizer, max_seq_length, verbose_logging=False, logger=None):
    max_term_num = 1
    max_sent_length, max_term_length = 0, 0
    ...
    for (example_index, example) in enumerate(examples):
        ...
        orig_to_tok_index = []
        all_doc_tokens = []
        example_tokens = tokenizer.tokenize(example.term_texts)
        ...
        polarity_label = [label_to_id[example.polarity]]
        label_masks = [1] * len(polarity_label)

        assert len(polarity_label) == max_term_num
        assert len(label_masks) == max_term_num
        ...
        features.append(
            InputFeatures(
                ...
                input_mask=input_mask,
                segment_ids=segment_ids,
                polarity_labels=polarity_label,
                label_masks=label_masks))
        unique_id += 1
    logger.info("Max sentence length: {}".format(max_sent_length))
    return features
...
RawFinalResultPolarity = collections.namedtuple("RawFinalResultPolarity",
                                        ["unique_id", "cls_pred", "span_masks"])
        ...
        if is_training:
            ...
            for start_index in start_indexes:
                for end_index in end_indexes:
                    ...
                    # this one is needed because otherwise contiguous spans get extracted as one
                    if length > max_answer_length:
                        continue
                    # ignore spans of 1 word
                    if length < 2:
                        continue
...
def convert_absa_data_polarity(dataset, verbose_logging=False):
    examples = []
    n_records = len(dataset)
    for i in range(n_records):
        words = dataset[i]['words']
        ...
        if term_texts != []:
            ...
            for j in range(len(polarities)):
                ...
                # un example por cada polarity
                example = PolarityExample(str(i), term_texts[j], new_polarities[j])
                examples.append(example)
